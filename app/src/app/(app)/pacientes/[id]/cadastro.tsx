...
import type { OpcaoProcedimentoInteresse, PacienteCadastro } from './tipos'
...
export function FormularioCadastro({
  ...
  hojeISO: string
  procedimentos: OpcaoProcedimentoInteresse[]
}) {
  ...
  const { status, erro, marcarAlterado } = useAutosaveForm({
    save: async () => {
      ...
      return salvarCadastro(new FormData(formRef.current))
    },
  })
  ...
  return (
    ...
    >
      ...
      <fieldset className="grid gap-4 sm:grid-cols-2">
        ...
        <label className="block space-y-1 sm:col-span-2">
          <span className="text-sm text-texto/80">Procedimento de interesse</span>
          <select
            name="procedimento_interesse_id"
            defaultValue={paciente.procedimento_interesse_id ?? ''}
            className={CAMPO}
          >
